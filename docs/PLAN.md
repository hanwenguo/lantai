...
`init` prompts when it has no `--library` and stdin, stdout, and stderr are all terminals, and asks nothing otherwise, so the flags are overrides rather than the ordinary path. `check` subsumes the earlier separate `health`: one command answers both which paths are in use and whether the library is intact. REST keeps `/health` as a cheap cached liveness probe, which is a different question. Because clap has no subcommand groups, `--help` renders a grouped listing of its own, which also names the `lantai-*` extensions found on `PATH`.
