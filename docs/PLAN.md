# Lantai 0.6.1: BibLaTeX-backed headless reference manager
...
`init` prompts when it has no `--library` and stdin, stdout, and stderr are all terminals, and asks nothing otherwise, so the flags are overrides rather than the ordinary path. The public REST field names are guarded by rejecting unknown query parameters and body fields and by a version handshake on connect, so a version skew fails loudly instead of silently returning an unfiltered list or a change that never happened. `check` subsumes the earlier separate `health`: one command answers both which paths are in use and whether the library is intact. REST keeps `/health` as a cheap cached liveness probe, which is a different question. Because clap has no subcommand groups, `--help` renders a grouped listing of its own, which also names the `lantai-*` extensions found on `PATH`.

Commands accept UUID or citation key. They use REST when the configured daemon is reachable and authenticated; otherwise they invoke the same locked catalog service directly. The reading commands `list`, `show`, and `collection list` emit JSON by default and accept `--format human`; other commands retain human-readable defaults with `--json` for automation. `export` emits the complete canonical file or a filtered selection to stdout/path.
...
Version 0.6.1 is complete when an unmodified Zotero Connector can save common web
